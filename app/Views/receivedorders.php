    ...
    <div class="w-full px-6 py-6 mx-auto">
      ...
      <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
        <div class="p-6 px-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
          <h6 class="mb-0">Commandes reçues</h6>
        </div>
              ...
              <?php foreach ($orders as $order) : ?>
                <li class="relative flex p-6 mb-2 border-0 rounded-t-inherit rounded-xl bg-gray-50">
                  <div class="flex flex-col">
                    ...
                    <span class="mb-2 leading-tight text-xs">Client: <span class="font-semibold text-slate-700 sm:ml-2"><?= $order->getCustomer()->getName() ?></span></span>
                  ...
                  <div class="ml-auto text-right">
                    ...
                    <?php if ($order->getStatus() == 'pending') : ?>
                      <form action="/order/processing" method="POST" class="inline-block">
                        <input type="hidden" name="order_id" value="<?= $order->getId() ?>">
                        <button type="submit" class="inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 text-slate-700"><i class="mr-2 fas fa-pencil-alt text-slate-700" aria-hidden="true"></i>
                          Mettre à en cours de traitement
                        </button>
                      </form>
                    <?php elseif ($order->getStatus() == 'processing') : ?>
                      <form action="/order/completed" method="POST" class="inline-block">
                        <input type="hidden" name="order_id" value="<?= $order->getId() ?>">
                        <button type="submit" class="inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 text-slate-700"><i class="mr-2 fas fa-pencil-alt text-slate-700" aria-hidden="true"></i>
                          Finaliser
                        </button>
